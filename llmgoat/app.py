...
from llmgoat.security_checkpoint import is_prompt_safe
...
def challenge_api(challenge_id):
    # Only allow A01 challenge for now
    if challenge_id != "a01-prompt-injection":
        return jsonify({"error": "Only A01 challenge is currently available."}), 404
    ...
    try:
        ...
        # Import and handle the A01 challenge
        challenge_module = importlib.import_module("llmgoat.challenges.a01_prompt_injection")
...
def parse_args():
    ...
    parser.add_argument("--host", "-h", type=str, default="0.0.0.0", help="Host for API server (e.g. '0.0.0.0')")
...
def main():
    ...
    # Only A01 is currently supported
    pass
